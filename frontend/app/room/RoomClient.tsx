...
const RoomSelection = ({ value, room, id, updateRoom, updateParticipant: updateParticipantLocal, diselection }: RoomSelectionProps) => {
  const [selected, setSelected] = useState(value ?? 'none');
  
  const handleChangeRoom = async (e: SelectChangeEvent<string>, participantId: string) => {
  ...
  return (
    ...
    >
      ...
      {room.map((r) => (
        ...
        >
          Room No: {r.roomNo}, Capacity: {r.capacity}, Available: {r.capacity - r.booked}
        </MenuItem>
      ))}
    </Select>
  );
...
export default function RoomClient({ participants: initialParticipants, rooms: initialRooms }: RoomClientProps) {
  ...
  const updateParticipantRoom = (targetRoomId: string, participantId: string) => {
  ...
  const updateRoomBooking = (roomId: string) => {
    ...
    if (updatedRoom) {
      dispatch(actions.updateRoom({ ...updatedRoom, booked: updatedRoom.booked + 1 }));
    }
  };
  ...
  const diselection = (roomId: string, participantId: string) => {
    ...
    if (targetRoom && targetRoom.booked > 0) {
      dispatch(actions.updateRoom({ ...targetRoom, booked: targetRoom.booked - 1 }));
    }

    // Update participant room assignment
    const targetParticipant = participantState.find(p => p._id === participantId);
    if (targetParticipant) {
      dispatch(actions.updateParticipant({
        ...targetParticipant,
        room: undefined,
        roomNo: undefined,
      }));
      ...
      {fellowships.map((fel) => {
        ...
        return (
          ...
          >
            ...
            <Table aria-label="fellowship participants table" size="small" sx={{ p: 4 }}>
              <TableHead>
                <TableRow>
              ...
              <TableBody>
                {participantsByFel.map((row) => (
                  ...
                  >
                    ...
                    <TableCell align="right">
                      <RoomSelection
                        value={typeof row?.roomNo === 'object' ? row?.roomNo?.roomNo : row?.roomNo}
                        room={roomState}
                        id={row._id}
